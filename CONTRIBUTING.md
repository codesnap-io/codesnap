...
1) Minimize the number of commits so that upstream master branch is as clean as possible
2) "git log" and copy hash from commit BEFORE the last one you intend to squash
3) "git rebase -i <paste hash>
4) Replace the word "pick" with "squash" on commits you want to merge with the commit above
5) For additional reference, check out this article
